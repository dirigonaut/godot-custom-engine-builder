...
os_builds=$OPERATING_SYSTEMS
# An option to create the godot tarball even if the checkout was skipped
# - For creating custom patch builds where the user manages the git state
create_tarball=0
...
export podman_run="${podman} run -it --rm --env BUILD_NAME=${BUILD_NAME} --env GODOT_VERSION_STATUS=${GODOT_VERSION_STATUS} --env NUM_CORES=${NUM_CORES} --env CLASSICAL=${build_classical} --env MONO=${build_mono} --env ARCHITECTURES=${ARCHITECTURES} --env OPERATING_SYSTEMS=${OPERATING_SYSTEMS} -v ${basedir}/godot-${godot_version}.tar.gz:/root/godot.tar.gz -v ${basedir}/mono-glue:/root/mono-glue -w /root/"
...
if printf '%s\0' "${os_builds[@]}" | grep -Fqwz -- "windows"; then
...
if printf '%s\0' "${os_builds[@]}" | grep -Fqwz -- "linux"; then
...
if printf '%s\0' "${os_builds[@]}" | grep -Fqwz -- "web"; then
...
if printf '%s\0' "${os_builds[@]}" | grep -Fqwz -- "macos"; then
...
if printf '%s\0' "${os_builds[@]}" | grep -Fqwz -- "android"; then
...
if printf '%s\0' "${os_builds[@]}" | grep -Fqwz -- "ios"; then
